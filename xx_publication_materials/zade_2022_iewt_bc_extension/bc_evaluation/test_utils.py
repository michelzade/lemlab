...
def setup_test_general(generate_random_test_data=False, test_config_path=None):
    ...
    if generate_random_test_data:
        init_random_market_data(db_obj=db_obj, bc_obj_market=bc_obj_clearing_ex_ante,
                                config=config, bc_obj_settlement=bc_obj_settlement)

    return config, db_obj, bc_obj_clearing_ex_ante, bc_obj_settlement


def init_random_market_data(db_obj, bc_obj_market, config, bc_obj_settlement):
    # Clear data from db and bc
    reset_all_market_tables(db_obj=db_obj, bc_obj_market=bc_obj_market, bc_obj_settlement=bc_obj_settlement)
    # Create list of random user ids and meter ids
    ids_users, ids_meters, ids_market_agents = setup_random_prosumers(db_obj=db_obj, bc_obj_market=bc_obj_market,
                                                                      n_prosumers=config["bc_performance_analysis"]["n_prosumers"])
    # Compute random market positions
    positions = create_random_positions(db_obj=db_obj,
                                        config=config,
                                        ids_user=ids_meters,
                                        n_positions=config["bc_performance_analysis"]["n_positions"]["max"],
                                        verbose=False)
...
def create_random_positions(db_obj, config, ids_user, n_positions=None, verbose=False):
    ...
    t_end = t_start + config['lem']['interval_clearing'] * config["bc_performance_analysis"]["n_ts_delivery"]
    # Range of time steps
    t_d_range = np.arange(t_start, t_end, config['lem']['interval_clearing'])
    position_number_range = np.arange(0, config["bc_performance_analysis"]["max_number_positions_ts_d"])
    # Create market positions (10 times the requested positions are computed because duplicates must be dropped)
    ...
    if position_number_range is not None:
        positions.loc[:, db_obj.db_param.NUMBER_POSITION] = random.choices(position_number_range, k=n_positions)
    else:
    ...
    if len(positions) > n_positions:
        # Sample as many positions as requested
        positions = positions.sample(n=n_positions)

    # Check whether number
    if verbose:
        print(f"{pd.Timestamp.now()}, {len(positions)} positions created")

    return positions


def clearing_ex_ante_test(generate_random_test_data):
...
def settlement_test(generate_random_test_data):
    config, db_obj, bc_obj_clearing_ex_ante, bc_obj_settlement = clearing_ex_ante_test(generate_random_test_data)
    ...
    # Settle market
    settle_market_db(config=config, db_obj=db_obj, ts_delivery_list=ts_delivery_list, path_sim=sim_path)
    settle_market_bc(config=config, bc_obj_settlement=bc_obj_settlement, ts_delivery_list=ts_delivery_list)

    return config, db_obj, bc_obj_clearing_ex_ante, bc_obj_settlement


def settle_market_db(config, db_obj, ts_delivery_list, path_sim):
    ...
    # Update balances with levies on db
...
def reset_all_market_tables(db_obj, bc_obj_market, bc_obj_settlement):
    # Clear data from db and bc
    db_obj.init_db(clear_tables=True, reformat_tables=True)
    bc_obj_market.clear_temp_clearing_data()
    bc_obj_market.clear_permanent_clearing_data()
    bc_obj_settlement.clear_settlement_data()


def reset_dynamic_market_data_tables(db_obj, bc_obj_market, bc_obj_settlement):
    """
    Resets all tables on db and bc except user info and meter info
    """
    # Database reset
...
def setup_random_prosumers(db_obj, bc_obj_market, n_prosumers):
    # Create list of random user ids and meter ids
    ids_users = create_user_ids(num=n_prosumers)
    ids_meter = create_user_ids(num=n_prosumers)
    ids_market_agents = create_user_ids(num=n_prosumers)

    tx_hash = None
    for z in range(len(ids_users)):
        # Register users on bc and db
        cols, types = db_obj.get_table_columns(db_obj.db_param.NAME_TABLE_INFO_USER, dtype=True)
        col_data = [ids_users[z], 1000, 0, 10000, 100, 'green', 10, 'zi', 0, ids_market_agents[z], 0, 2147483648]
        ...
        col_data = [ids_meter[z], ids_users[z], "0", "virtual grid meter", '0' * 10, 'green', 0,
                    2147483648,
                    'test']
        if any([type(data) != typ for data, typ in zip(col_data, types)]):
    ...
    return ids_users, ids_meter, ids_market_agents


if __name__ == '__main__':
    random.seed(0)
    # setup_test_general(generate_random_test_data=True)
    # clearing_ex_ante_test(generate_random_test_data=True)
    # settlement_test(generate_random_test_data=True)
